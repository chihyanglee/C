...
void longestSub(int *ARRAY, int ARRAY_LENGTH, int **RESULT,
                int *RESULT_LENGTH) { // RESULT and RESULT_LENGTH will be
                                      // modified by their pointers
  ...
  if (ARRAY_LENGTH <= 1) {
    ...
    *RESULT_LENGTH = ARRAY_LENGTH;
    ...
    for (i = 1; i < ARRAY_LENGTH; i++) {
      ...
      if (ARRAY[i] >= PIVOT) {
        TEMPORARY_ARRAY_LENGTH++;
        TEMPORARY_ARRAY = (int *)realloc(TEMPORARY_ARRAY,
