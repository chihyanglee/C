...
struct Graph {
  ...
  int *visited;
  struct node *
      *adjLists; // we need int** to store a two dimensional array. Similary, we
                 // need struct node** to store an array of Linked lists
};
...
void topologicalSortHelper(int vertex, struct Graph *graph,
                           struct Stack *stack) {
