...
node *insert(node *root, int data) {
  ...
  if (root == NULL)
    root = newNode(data);
  // If it isn't null and the input key is greater than the root key, insert in
  // the right leaf
...
node *delete (node *root, int data) {
  ...
  // termination condition
    ...
    // Case 2: the root has one leaf, make the leaf the new root and remove the
    // old root
    ...
    // Case 3: the root has 2 leaves, find the greatest key in the left subtree
    // and switch with the root's
    else {
...
int height(node *root) {
  ...
  if (root == NULL)
    ...
    // Get the height from both left and right subtrees to check which is the
    // greatest
